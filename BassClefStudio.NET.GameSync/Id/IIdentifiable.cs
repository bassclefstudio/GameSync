...
namespace BassClefStudio.NET.GameSync.Id
{
    ...
    public static class GuidExtensions
    {
        ...
        /// <summary>
        /// Returns a <see cref="bool"/> value indicating whether a <typeparamref name="T"/> with a given <see cref="Guid"/> ID exists in a collection of <typeparamref name="T"/> <see cref="IIdentifiable"/> objects.
        /// </summary>
        /// <typeparam name="T">The type of <see cref="IIdentifiable"/> to look through.</typeparam>
        /// <param name="items">The collection of <typeparamref name="T"/> to search.</param>
        /// <param name="id">The unique <see cref="Guid"/> of the desired item.</param>
        public static bool Contains<T>(this IEnumerable<T> items, Guid id) where T : IIdentifiable
        {
            return items.Any(t => t.Id == id);
        }
    }
}
